...
#[structopt(name="ciruela put-file", about="
    Downloads a directory index from a remote location, puts a file it and
    syncs an equivalent image with an updated file back to the cluster.
    This works both for adding and replacing files.
")]
...
pub fn cli(gopt: GlobalOptions, mut args: Vec<String>) -> ! {
    args.insert(0, String::from("ciruela put-file"));  // temporarily
    let opts = PutFileOptions::from_iter(args);
